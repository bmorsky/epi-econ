...
function epiecon_ode(du, u, p, t)
    ...
    initial_guess = [200, 200, 200, 200, 200, 200, 200, 200, 200, 2.0, 00001]
...
for n=1:101
    β₁ = 0.35+0.1*(n-1)/100
    #    α,   β₁,  β₂,  βᵤ,  γ,   λ₁,     λ₂,     μ,     ρ,    b,    c₁,  c₂,  d, r,    y₁,  y₂,  bg
    pbetae = [0.5, β₁, 0.3, 0.2, 1/7, 0.0033, 0.0033, 0.072, 0.01, 0.71, 0.1, 0.1, 1, 0.02, 1.1, 1.0, 0.0]
    p=pbetae
    function save_theta!(u, t, integrator)
        α,β₁,β₂,βᵤ,γ,λ,μ,ρ,b,c₁,c₂,d,r,y₁,y₂,bg = p
        S₁, I₁, R₁, S₂, I₂, R₂, Sᵤ, Iᵤ, Rᵤ = u
    ...
    prob = ODEProblem(epiecon_ode, u₀, tspan, pbetae)
    sol = solve(prob,saveat=0:1:T, callback=save_cb)
    E[n] = sol[1,end]+sol[2,end]+sol[3,end]
    G[n] = sol[4,end]+sol[5,end]+sol[6,end]
    U[n] = sol[7,end]+sol[8,end]+sol[9,end]
...
sweep_betae=plot(0.35:0.001:0.45,[E,G,U,Θe,Θg],label=[L"E" L"G" L"U" L"\Theta_e" L"\Theta_g"],xlabel=L"\beta_e",legend=:outerright)
...
for n=1:101
    βᵤ = 0.15+0.1*(n-1)/100
    #    α,   β₁,  β₂,  βᵤ,  γ,   λ₁,     λ₂,     μ,     ρ,    b,    c₁,  c₂,  d, r,    y₁,  y₂,  bg
    pbetau = [0.5, 0.4, 0.3, βᵤ, 1/7, 0.0033, 0.0033, 0.072, 0.01, 0.71, 0.1, 0.1, 1, 0.02, 1.1, 1.0, 0.0]
    p=pbetau
    function save_theta!(u, t, integrator)
        α,β₁,β₂,βᵤ,γ,λ,μ,ρ,b,c₁,c₂,d,r,y₁,y₂,bg = p
        S₁, I₁, R₁, S₂, I₂, R₂, Sᵤ, Iᵤ, Rᵤ = u
    ...
    prob = ODEProblem(epiecon_ode, u₀, tspan, pbetau)
    sol = solve(prob,saveat=0:1:T, callback=save_cb)
    E[n] = sol[1,end]+sol[2,end]+sol[3,end]
    G[n] = sol[4,end]+sol[5,end]+sol[6,end]
    U[n] = sol[7,end]+sol[8,end]+sol[9,end]
...
sweep_betau=plot(0.15:0.001:0.25,[E,G,U,Θe,Θg],label=[L"E" L"G" L"U" L"\Theta_e" L"\Theta_g"],xlabel=L"\beta_u",legend=:outerright)
...
for n=1:101
    γ = 0.05 + 0.002*(n-1)
    #    α,   β₁,  β₂,  βᵤ,  γ,   λ₁,     λ₂,     μ,     ρ,    b,    c₁,  c₂,  d, r,    y₁,  y₂,  bg
    pgamma = [0.5, 0.4, 0.3, 0.2, γ, 0.0033, 0.0033, 0.072, 0.01, 0.71, 0.1, 0.1, 1.0, 0.02, 1.1, 1.0, 0.0]
    p=pgamma
    function save_theta!(u, t, integrator)
        α,β₁,β₂,βᵤ,γ,λ,μ,ρ,b,c₁,c₂,d,r,y₁,y₂,bg = p
        S₁, I₁, R₁, S₂, I₂, R₂, Sᵤ, Iᵤ, Rᵤ = u
    ...
    prob = ODEProblem(epiecon_ode, u₀, tspan, pgamma)
    sol = solve(prob,saveat=0:1:T, callback=save_cb)
    E[n] = sol[1,end]+sol[2,end]+sol[3,end]
    G[n] = sol[4,end]+sol[5,end]+sol[6,end]
    U[n] = sol[7,end]+sol[8,end]+sol[9,end]
...
sweep_gamma=plot(0.05:0.002:0.25,[E,G,U,Θe,Θg],label=[L"E" L"G" L"U" L"\Theta_e" L"\Theta_g"],xlabel=L"\gamma",legend=:outerright)
...
sweep_rho=plot(0:0.003:0.3,[E,G,U,Θe,Θg],label=[L"E" L"G" L"U" L"\Theta_e" L"\Theta_g"],xlabel=L"\rho",legend=:outerright)

plot(sweep_betae,sweep_betag,sweep_betau,sweep_gamma,sweep_rho,sweep_b,layout=(2,4))
savefig("sweep.pdf")
